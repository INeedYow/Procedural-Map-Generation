...
public class RoomChecker
{
    ...
    {   // wallSize * 2.f로 딱 맞게 체크하면 다중 연결 방이 생기지 않음
        Collider2D[] colls = new Collider2D[2];
        int collisionCount = Physics2D.OverlapBoxNonAlloc(position, new Vector2(room.width + MapGenerator.Instance.wallSize * 1.8f, room.height + MapGenerator.Instance.wallSize * 1.8f), 0f, colls, LayerMask.GetMask("Room"));
    ...
    public static void LinkRoom(Room a, Room b, EDirection directionA2B)
    {
        ...
        MapGenerator.Instance.CreatePath(a, b, directionA2B);
    }
}
